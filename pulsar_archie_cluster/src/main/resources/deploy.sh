#!/usr/bin/env bash
# Script to deploy pulsar in pulsar-archie-cluster.
# More info 
...
testZK() {
	...
	pulsar-2/bin/pulsar zookeeper-shell -server 127.0.0.1:22181 set /testzk pulsar_archie_cluster >/dev/null 2>&1
	pulsar-1/bin/pulsar zookeeper-shell -server 127.0.0.1:12181 get  /testzk > ./zktest 2>&1

	if [[ -f ./zktest && $(grep "pulsar_archie_cluster" ./zktest)="pulsar_archie_cluster" ]]; then
		${echo_with_date} "[6/9][√] Your Zookeeper archie-cluster is all ready."
		rm ./zktest
		pulsar-1/bin/pulsar zookeeper-shell -server $zk_server delete /testzk >/dev/null 2>&1
	else
		${echo_with_date} "[6/9][x] Your Zookeeper archie cluster test failed. Please check it manually."
		exit 1
	fi
}
...
replaceBrokerConf() {
	...
	# clusterName=
	${sed_i} 's|clusterName=|clusterName=pulsar_archie_cluster|' pulsar-1/conf/broker.conf
	${sed_i} 's|clusterName=|clusterName=pulsar_archie_cluster|' pulsar-2/conf/broker.conf
	${sed_i} 's|clusterName=|clusterName=pulsar_archie_cluster|' pulsar-3/conf/broker.conf
...
getPulsarMetaDate() {
	metadata_info=$(pulsar-1/bin/pulsar zookeeper-shell -server 127.0.0.1:12181 get  /admin/clusters/pulsar_archie_cluster | grep serviceUrl)
...
crateTenantAndNamespace() {
	...
	${echo_with_date} "[ list brokers ] -> pulsar-1/bin/pulsar-admin brokers list pulsar_archie_cluster"
	pulsar-1/bin/pulsar-admin brokers list pulsar_archie_cluster
	...
	${echo_with_date} "[ create local cluster ] -> pulsar-1/bin/pulsar-admin clusters create pulsar_archie_cluster --url http://127.0.0.1:18080/"	
	pulsar-1/bin/pulsar-admin clusters create  pulsar_archie_cluster --url http://127.0.0.1:18080/
	${echo_with_date} "[ create tenant ] -> pulsar-1/bin/pulsar-admin tenants create t1 -c pulsar_archie_cluster"	
	pulsar-1/bin/pulsar-admin tenants create t1 -c pulsar_archie_cluster
	${echo_with_date} "[ create tenant/namespaces ] -> pulsar-1/bin/pulsar-admin namespaces create t1/ns1 -c pulsar_archie_cluster"
	pulsar-1/bin/pulsar-admin namespaces create t1/ns1 -c pulsar_archie_cluster
...
####### => Let's go
############################
${echo_with_date} "====> Start to (re)deploy the archie-cluster of pulsar <===="
...
# 2. Deploy archie-cluster of Zookeeper
# - Configure log4j2 for flush log immediatelly
replaceLog4j2
# - Configure Zookeeper 
replaceZookeeperConf
# - Start Zookeeper archie-cluster
startZK
# - Test Zookeeper
testZK
...
# 4. Deploy archie-cluster of BookKeeper(bookies)
